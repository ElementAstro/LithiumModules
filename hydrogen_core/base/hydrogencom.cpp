...
int fs_sexa(char *out, double a, int w, int fracbase)
{
    ...
    n = static_cast<unsigned long>(a * fracbase + 0.5);
    d = n / fracbase;
    f = n % fracbase;

    if (isneg && d == 0)
        out += snprintf(out, MAXHYDROGENFORMAT, "%*s-0", w - 2, "");
    else
        out += snprintf(out, MAXHYDROGENFORMAT, "%*d", w, isneg ? -d : d);

    switch (fracbase)
    {
    case 60: // dd:mm
        m = f / (fracbase / 60);
        out += snprintf(out, MAXHYDROGENFORMAT, ":%02d", m);
        break;
    case 600: // dd:mm.m
        out += snprintf(out, MAXHYDROGENFORMAT, ":%02d.%1d", f / 10, f % 10);
        break;
    case 3600: // dd:mm:ss
        m = f / (fracbase / 60);
        s = f % (fracbase / 60);
        out += snprintf(out, MAXHYDROGENFORMAT, ":%02d:%02d", m, s);
        break;
    case 36000: // dd:mm:ss.s
        m = f / (fracbase / 60);
        s = f % (fracbase / 60);
        out += snprintf(out, MAXHYDROGENFORMAT, ":%02d:%02d.%1d", m, s / 10, s % 10);
        break;
    case 360000: // dd:mm:ss.ss
        m = f / (fracbase / 60);
        s = f % (fracbase / 60);
        out += snprintf(out, MAXHYDROGENFORMAT, ":%02d:%02d.%02d", m, s / 100, s % 100);
        break;
    default:
        return -1;
    }

    return static_cast<int>(out - out0);
}
               ...
               double *dp)       /* cracked value, if return 0 */
{
    ...
    */
    ...
    hydrogen_locale_C_numeric_pop(orig);
...
int numberFormat(char *buf, const char *format, double value)
{
    ...
    else
    {
        ...
        return (snprintf(buf, MAXHYDROGENFORMAT, format, value));
    }
}
...
{
    va_list ap;
    /* JM: Since all HYDROGEN's stderr are timestampped now, we don't need to time stamp ID Log */
