...
{
    char message[MAXHYDROGENMESSAGE];
    if (fmt)
    {
        vsnprintf(message, MAXHYDROGENMESSAGE, fmt, ap);
...
{
    userio_printf(io, user, "<getProperties version='%g'", HYDROGENV); // safe
    // special case for HYDROGEN::BaseClient::listenHYDROGEN HYDROGEN::BaseClientQt::connectServer
    if (dev && dev[0])
    {
    ...
    const userio *io, void *user, int ctag)
{
    ...
    if (ctag == 0)
    {
        userio_prints(io, user, "<HYDROGENDriver>\n");
    }
    /* Closing tag */
    else
    {
